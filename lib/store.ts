...
// Initialized with Mia's seed data so SSR + first paint are consistent.
...
function recompute(checkins: StoredCheckin[], seed: string) {
  const attributes = checkins.reduce<Attributes>(
  ...
  const { overlays } = computeAvatarState(attributes, eggs, seed);
  return { attributes, eggs, overlays };
}
